        ...
        with open(path, "rb") as f:
            ...
            if remainder:
                ...
                self.block(x, chunk_y,
                           ...
                           buf)
    def draw_image_ram(self, infow, x=0, y=0, w=128, h=128):
        x2 = x + w - 1
        y2 = y + h - 1
        if self.is_off_grid(x, y, x2, y2):
            return
        ...
        if chunk_count:
            for c in range(0, chunk_count):
                buf = self.noc(infow,chunk_size)
                self.block(x, chunk_y,
                            x2, chunk_y + chunk_height - 1,
                            buf)
                chunk_y += chunk_height
        if remainder:
            buf = self.noc(infow,remainder * w * 2)
            self.block(x, chunk_y,
                        x2, chunk_y + remainder - 1,
                        buf)
    def noc(self,strx,lenx):
        q=strx[self.lox:lenx+self.lox]
        self.lox+=lenx
        return q
